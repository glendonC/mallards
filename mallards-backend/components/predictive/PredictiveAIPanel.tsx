...
const PredictiveAIPanel: React.FC<PredictiveAIPanelProps> = ({
  isOpen,
  onClose,
  data,
  customColors
  ...
  const analyzeData = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/analyze/predictive', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/chat/predictive', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {error ? (
              <Alert variant="destructive" className="mb-4">
                ...
                <AlertDescription>
                  {typeof error === 'object' ? 'Error analyzing data' : error}
                </AlertDescription>
              </Alert>
            ) : isAnalyzing ? (
